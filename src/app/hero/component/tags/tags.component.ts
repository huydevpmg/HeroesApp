...
export class TagsComponent implements OnInit, OnChanges {
  ...
  private syncSelectedTags(): void {
    if (!this.currentHeroTags || this.allTags.length === 0) {
      return;
    }
    ...
    if (this.heroIds.length > 1) {
      this.store
        ...
        .pipe(take(1))
        .subscribe((heroes) => {
          const selectedHeroes = heroes.filter((h) =>
            this.heroIds.includes(h._id)
          );
  ...
  toggleTag(tag: Tag): void {
    ...
    if (isChecked) {
      ...
      this.store.dispatch(
        removeTagFromHero({ heroIds: this.heroIds, tag: tag.name })
      );
      this.toastr.success(`Tag "${tag.name}" removed successfully!`);
    } else {
      this.selectedTags.push(tag);
      this.store.dispatch(
        addTagToHero({ heroIds: this.heroIds, tag: tag.name })
      );
